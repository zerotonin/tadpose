...
class FlexibleVelocityAnalysis:
    """
    A flexible class to perform velocity analysis on tadpole data,
    ...
    def load_data(self, csv_files):
        ...
        required_columns = {'trial_id', 'well_number', 'well_type_id', 'thrust_mm_s', 'slip_mm_s', 'yaw_rad_s', 'label'}
    ...
    def calculate_velocity(self):
        """
        Calculate combined velocity from thrust, slip, and yaw.

        Adds a new column 'velocity' to the DataFrame.
        """
        print("\nCalculating combined velocity from thrust, slip, and yaw...")
        # Assuming velocity is the Euclidean norm of thrust, slip, and yaw
        self.data['velocity'] = np.sqrt(
            self.data['thrust_mm_s']**2 +
            self.data['slip_mm_s']**2 +
            self.data['yaw_rad_s']**2
        )
        print("Combined velocity calculation completed.")
        # Debugging: Print a sample of the velocity calculations
        print("\nSample velocity calculations:")
        print(self.data[['thrust_mm_s', 'slip_mm_s', 'yaw_rad_s', 'velocity']].head())

    def calculate_mean_velocity(self):
        """
        Calculate the mean velocity for each trial_id, well_number, and Group.
        Assign internal IDs for pairing if needed.

        Returns:
            pd.DataFrame: DataFrame with trial_id, well_number, Group, mean_velocity, and internal_id.
        """
        # Group by trial_id, well_number, and Group and calculate mean velocity
        mean_velocity = self.data.groupby(['trial_id', 'well_number', 'Group'])['velocity'].mean().reset_index()
        mean_velocity.rename(columns={'velocity': 'mean_velocity'}, inplace=True)

        # Assign internal_id for pairing
        mean_velocity['internal_id'] = mean_velocity.groupby(['Group', 'well_number'])['trial_id'].rank(method='first').astype(int)

        # Debugging: Print a sample of the mean velocities with internal IDs
        print("\nMean velocities with internal IDs:")
        print(mean_velocity.head())
    ...
    def perform_statistical_tests(self, mean_velocity_df, comparisons, output_directory):
        """
        Perform statistical tests based on comparisons and normality.

        Parameters:
            mean_velocity_df (pd.DataFrame): DataFrame with mean velocities and groups.
        ...
        for comparison in comparisons:
            ...
            data = mean_velocity_df[mean_velocity_df['Group'].isin(groups)].copy()

            # Recalculate group normality for this subset
            shapiro_results = []
            group_normality = {}
            for group in groups:
                group_data = data[data['Group'] == group]['mean_velocity']
            ...
            if test_name == 'Friedman test':
                ...
                data = data.groupby(['well_number', 'Group'])['mean_velocity'].mean().reset_index()
                pivot_data = data.pivot(index='well_number', columns='Group', values='mean_velocity')
                ...
                data = data.groupby(['well_number', 'internal_id', 'Group'])['mean_velocity'].mean().reset_index()
                pivot_data = data.pivot_table(index=['well_number', 'internal_id'], columns='Group', values='mean_velocity')
                pivot_data = pivot_data.dropna()
                if pivot_data.shape[0] < 2:
                ...
                melted_data = pivot_data.reset_index().melt(id_vars=['well_number', 'internal_id'], value_vars=groups,
                                                            var_name='Group', value_name='mean_velocity')
                ...
                aovrm = AnovaRM(melted_data, 'mean_velocity', 'subject_id', within=['Group'])
                res = aovrm.fit()
                ...
                for group1, group2 in comparisons:
                    data1 = melted_data[melted_data['Group'] == group1]['mean_velocity']
                    data2 = melted_data[melted_data['Group'] == group2]['mean_velocity']
                ...
                data_grouped = data.groupby(['well_number', 'internal_id', 'Group'])['mean_velocity'].mean().reset_index()
                merged_data = data_grouped.pivot_table(index=['well_number', 'internal_id'], columns='Group', values='mean_velocity')
                ...
                data1 = data[data['Group'] == groups[0]]['mean_velocity']
                data2 = data[data['Group'] == groups[1]]['mean_velocity']
                ...
                model = ols('mean_velocity ~ C(Group)', data=data).fit()
                ...
                mc = sm.stats.multicomp.MultiComparison(data['mean_velocity'], data['Group'])
                ...
                data_groups = [data[data['Group'] == group]['mean_velocity'] for group in groups]
                ...
                dunn_results = sp.posthoc_dunn(data, val_col='mean_velocity', group_col='Group', p_adjust='bonferroni')
        ...
        stat_output_path = os.path.join(output_directory, 'statistical_tests_results_velocity.csv')
    ...
    def plot_results(self, mean_velocity_df, group_stats_df, sig_pairs, output_directory, groups_to_include):
        """
        Plot mean velocities for each group with mean and 95% CI,
        ...
        mean_velocity_df['Group'] = pd.Categorical(mean_velocity_df['Group'], categories=groups_to_include, ordered=True)
        group_stats_df['Group'] = pd.Categorical(group_stats_df['Group'], categories=groups_to_include, ordered=True)

        plt.figure(figsize=(10, 8))
        sns.stripplot(x='Group', y='mean_velocity', data=mean_velocity_df,
        ...
        for idx, row in group_stats_df.iterrows():
            ...
            print(f"Group '{group}': Mean Velocity={mean:.4f} mm/s, 95% CI=({ci_lower:.4f}, {ci_upper:.4f}) mm/s, N={n}")
            # Create a single-row DataFrame of mean velocities
            single_row_df = pd.DataFrame([{
                "Group": group,
                "Mean Velocity (mm/s)": mean,
                "lower_95_ci": ci_lower,
                "upper_95_ci": ci_upper,
                "se": standard_error,
                "N": n
            }])
            mean_velocity_dfs.append(single_row_df)
        # Add significance annotations
        if sig_pairs:
            y_max = mean_velocity_df['mean_velocity'].max()
            y_min = mean_velocity_df['mean_velocity'].min()
        ...
        plt.title('Mean Velocity by Group')
        plt.xlabel('Group')
        ...
        # Save plot
        plot_filename = 'mean_velocity_by_group.png'
        ...
        mean_velocity_df_to_save = pd.concat(mean_velocity_dfs, ignore_index=True)

        # Define the path for the CSV file
        csv_output_path = os.path.join(output_directory, 'mean_velocity_by_group.csv')
        
        # Save the DataFrame to CSV
        mean_velocity_df_to_save.to_csv(csv_output_path, index=False)
        logging.info(f"Mean velocity by group saved to {csv_output_path}")




    def run_analysis(self, categories, comparisons, groups_to_include, output_directory):
        """
        Execute the full analysis: assign groups, calculate velocities, compute mean velocities,
        perform statistical tests, plot results, and save tests.
        ...
        mean_velocity_df = self.calculate_mean_velocity()

        # Perform statistical tests and get significant pairs and group stats
        sig_pairs, group_stats_df = self.perform_statistical_tests(mean_velocity_df, comparisons, output_directory)

        # Plot results
        self.plot_results(mean_velocity_df, group_stats_df, sig_pairs, output_directory, groups_to_include)
...
output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/velocity_analysis/sep_18_k_36/comparison_of_groups_plots/PTZ_combined_reps_different_levels'

# Path to your new CSV file with velocity data
csv_file_path_velocity = '/projects/sciences/zoology/geurten_lab/tadpole_project/cluster_analysis/sep_18_k_36/sep18_davies_bouldin_cleaned_tail_base_x_VELOCITY_tadpole_ids_trial_ids_well_type_ids_and_labels.csv'

# Initialize the velocity analysis
velocity_analysis = FlexibleVelocityAnalysis(csv_file_path_velocity)

# Run the analysis
velocity_analysis.run_analysis(
...
# output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/velocity_analysis/sep_18_k_36/comparison_of_groups_plots/bae_4ap_vpa'
...
# output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/velocity_analysis/sep_18_k_36/comparison_of_groups_plots/4ap_base_nd2_nd2_edit'
...
# # Define the output directory
# output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/velocity_analysis/sep_18_k_36/comparison_of_groups_plots/PTZ_rep_1_different_levels'
...
# # Define the output directory
# output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/velocity_analysis/sep_18_k_36/comparison_of_groups_plots/PTZ_rep_2_different_levels'
...
# # Define the output directory
# output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/velocity_analysis/sep_18_k_36/comparison_of_groups_plots/PTZ_rep_3_different_levels'
...
# Define the output directory
output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/velocity_analysis/sep_18_k_36/comparison_of_groups_plots/PTZ_vs_4-AP_vs_ND2'
