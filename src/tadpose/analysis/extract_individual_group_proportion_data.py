...
class FlexibleClusterAnalysis:
    ...
    def run_analysis(self, cluster_label_columns, categories, comparisons, groups_to_include, output_directory):
        ...
        # Create folder for the cluster label column
            ...
            # Initialize list to collect all proportion_dfs for this cluster_label_column
            all_proportion_dfs = []
            ...
            for cluster_label in cluster_labels:
                ...
                # Add a 'cluster_label' column to differentiate between clusters
                proportion_df['cluster_label'] = cluster_label

                # Append to the list
                all_proportion_dfs.append(proportion_df)

                # **Reset stat_test_results for each cluster label**
                self.stat_test_results = []

                # Perform statistical tests and get significant pairs and group stats
                sig_pairs, group_stats_df = self.perform_statistical_tests(proportion_df, comparisons, cluster_output_dir)

                # Plot results
                self.plot_results(proportion_df, group_stats_df, sig_pairs, cluster_output_dir, groups_to_include, cluster_label)

            # After processing all cluster_labels for this cluster_label_column,
            # concatenate all_proportion_dfs into a single DataFrame
            if all_proportion_dfs:
                combined_proportion_df = pd.concat(all_proportion_dfs, ignore_index=True)

                # Define the filename for the combined proportion data
                combined_csv_filename = f'combined_proportion_{cluster_label_column}.csv'
                combined_csv_path = os.path.join(cluster_output_dir, combined_csv_filename)

                try:
                    # Save the combined DataFrame to CSV
                    combined_proportion_df.to_csv(combined_csv_path, index=False)
                    logging.info(f"Combined proportion data for '{cluster_label_column}' saved to {combined_csv_path}")
                except Exception as e:
                    logging.error(f"Failed to save combined proportion data for '{cluster_label_column}' to {combined_csv_path}: {e}")
            else:
                logging.warning(f"No cluster labels found for '{cluster_label_column}'. No proportion data saved.")
