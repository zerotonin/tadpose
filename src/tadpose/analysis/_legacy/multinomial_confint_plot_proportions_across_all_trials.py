...
class TadpoleClusteringAnalysis:
    ...
    def plot_cluster_proportions(self, cluster_columns, graph_titles, output_dir, group_criteria, group_labels):
        ...
        for i, col in enumerate(cluster_columns):
            ...
            # Calculate proportions and confidence intervals for each group and align to the common index
            group_proportions = {}
            ci_lows = {}
            ci_ups = {}
            max_proportions = {}
            trial_counts = {}  # Dictionary to store unique trial_id counts for each cluster
            ...
            for j, criteria in enumerate(group_criteria):
                ...
                # Calculate proportions and reindex
                counts = group_data[col].value_counts().sort_index().reindex(common_index, fill_value=0)
                proportions = counts / counts.sum()
                group_proportions[group_labels[j]] = proportions
                
                # Calculate confidence intervals
                ci_low, ci_up = self.multinomial_proportions(counts)
                ci_lows[group_labels[j]] = ci_low.clip(0, 1)  # Ensure bounds are between 0 and 1
                ci_ups[group_labels[j]] = ci_up.clip(0, 1)

                # Calculate the number of unique trial_id for each cluster label
                trial_counts[group_labels[j]] = group_data.groupby(col)['trial_id'].nunique().reindex(common_index, fill_value=0).to_dict()

                # Calculate max proportion for each cluster across all groups
                if j == 0:
                    max_proportions = proportions.to_frame(name=group_labels[j])
                else:
                    max_proportions[group_labels[j]] = proportions
            ...
            max_proportions['degree_of_10'] = np.floor(np.log10(max_proportions['max_proportion'])).astype(int)
            ...
            proportions_df = pd.DataFrame(group_proportions).T  # Transpose to have groups as rows
            ci_lows_df = pd.DataFrame(ci_lows).T
            ci_ups_df = pd.DataFrame(ci_ups).T
            ...
            for cluster_group in cluster_groups:
                ...
                nrows = 1 if num_subplots <= 3 else (num_subplots + 1) // 3  # Fix nrows calculation
                ...
                all_y_values = proportions_df[cluster_group].values.flatten()
                ...
                for subplot_idx, ax in enumerate(axes):
                    ...
                    for cluster_label in sub_group:
                        # Calculate the number of unique trials for this cluster label
                        trial_count = trial_counts[group_labels[j]].get(cluster_label, 0)

                        # Label with cluster number and number of unique trials
                        label = f'Cluster {cluster_label} (n: {trial_count})'

                        ax.plot(
                            proportions_df.index,  # x-axis: group labels
                            proportions_df[cluster_label],  # y-axis: proportions of the current cluster
                            marker='o',
                            label=label  # Label for the legend
                        )

                        # Adding the confidence intervals as a shaded area
                        ax.fill_between(
                            proportions_df.index,
                            ci_lows_df[cluster_label],
                            ci_ups_df[cluster_label],
                            alpha=0.3  # Slightly increase transparency for visibility
                        )
                    ...
                    ax.set_xticklabels(proportions_df.index, rotation=45)
                    ax.set_yscale('log')
...
#     {'tadpole_id': 5}
# ]
# group_labels = ['Baseline', '10mM_PTZ', '6mM_PTZ', '3mM_PTZ', '1mM_PTZ', 'Eelfa2_g15_3mM', 'Eelfa2_g15_5mM', 'Eelfa2_g35', 'EElfa2_g15']
...
cluster_columns = ['label', 'agglom_4' , 'agglom_7']
graph_titles = ['Distribution of base 34 clusters', 'Distribution of 3 Cluster Super-Group', 'Distribution of 7 Cluster Super-Group']
output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/cluster_analysis/sep_18_k_36/multinomial_confint_plots'
# output_directory = '/projects/sciences/zoology/geurten_lab/tadpole_project/cluster_analysis/aug_22_k_34/biologial_group_cluster_frequency'
